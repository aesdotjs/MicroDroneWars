...
export class PlanePhysicsController extends BasePhysicsController {
    ...
    public update(deltaTime: number, input: PhysicsInput): void {
        ...
        const velLength = lookVelocity.norm();
        
        if (velLength > 0.1) {
            lookVelocity.scale(1/velLength);
            
            const rotStabVelocity = new CANNON.Quaternion();
            const axis = new CANNON.Vec3();
            const dot = new CANNON.Vec3(forward.x, forward.y, forward.z).dot(lookVelocity);
            
            const clampedDot = Math.max(-1, Math.min(1, dot));
            const angle = Math.acos(clampedDot);
            
            if (angle > 0.001) {
                axis.cross(new CANNON.Vec3(forward.x, forward.y, forward.z), lookVelocity);
                const axisLength = axis.norm();
